...
Content-Based Router (CBR) is a pattern from the https://www.enterpriseintegrationpatterns.com[Enterprise Integration Patterns]. You will use CBR when you need to filter some result and send to a different target based in the filter criteria.
