...
Every time you hit the HTTP endpoint the message *"getting all orders entries from database"* should appear in the log. It doesn't return anything yet because nothing has implemented to return to the caller. For now, don't worry about it.
...
All the maven dependencies are already included for in `pom.xml`. In this lab you will be using an embedded database, so don't worry about any installation process.
